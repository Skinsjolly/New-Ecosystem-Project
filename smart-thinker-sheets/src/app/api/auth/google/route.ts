...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user) {
      ...
      user = await prisma.user.create({ data: { email, username: finalUsername, googleId, avatar } })
    } else if (!user.googleId) {
      user = await prisma.user.update({ where: { id: user.id }, data: { googleId, avatar: avatar || user.avatar } })
    }
    const token = generateToken(user)
    const response = NextResponse.json({ user: { id: user.id, username: user.username, email: user.email, avatar: user.avatar }, token })
